...
from .models import Vote, VoteBackup, Voters
...
def create_vote(request):
    ...
    private_key = '1234'
    public_key = '1234'
    selected_party_id = 'nota'

    curr = timezone.now()
    ballot = f'{uuid}|{selected_party_id}|{curr.timestamp()}'

    # Calculate the hash of the ballot using SHA3-256
    from Crypto.Hash import SHA3_256
    ballot_hash = SHA3_256.new(ballot.encode()).hexdigest()

    # Simulate a successful verification
    status = [True, 'Your vote verified and Ballot is signed successfully.',
              ballot_hash, 'signature']

    # if status is not None and len(status) >= 4 and status[0]:
        # try:
    Vote(uuid=uuid, vote_party_id=selected_party_id, timestamp=curr).save()
    VoteBackup(uuid=uuid, vote_party_id=selected_party_id,
                       timestamp=curr).save()
    voter = Voters.objects.get(uuid=uuid)
    voter.vote_done = True
    voter.save()
        # except Exception as e:
        #     return JsonResponse({'success': False, 'status': 'Failed to save vote. ' + str(e)})
    ...
    return JsonResponse({'success': True, 'status': 'Vote successfully recorded.', 'html': html})
...
# ------------- Save vote in database ------------------------
    ...
    uuid = request.session.get('10')
    private_key = request.GET.get('1234')
    public_key = request.session.get('1234')
    selected_party_id = request.GET.get('nota')

    # Check if any of the key values is missing
    if not (uuid and private_key and public_key and selected_party_id):
        return JsonResponse({'success': False, 'status': 'Invalid input data.'})
    ...
    # status = verify_vote(private_key, public_key, ballot_hash)
    status = True
...
def create_block():
    ...
    non_sealed_votes = Vote.objects.all().filter(
        block_id=None).order_by('timestamp')
    non_sealed_votes_BACKUP = VoteBackup.objects.all().filter(
        block_id=None).order_by('timestamp')
    ...
    for _ in range(number_of_blocks):
        ...
        root = MerkleTools()
        root.add_leaf(
            [f'{tx.uuid}|{tx.vote_party_id}|{tx.timestamp}' for tx in block_transactions], True)
        root.make_tree()
        ...
        while True:
            enc = f'{prev_hash}{merkle_h}{nonce}{timestamp.timestamp()}'.encode(
                'utf-8')
        ...
        # Create the block
        curr_block_id += 1
        Block(id=curr_block_id, prev_hash=prev_hash, merkle_hash=merkle_h,
              this_hash=h, nonce=nonce, timestamp=timestamp).save()
        ...
        ts_data['status'] = str(curr_block_id - last_block_id) + ' blocks have been mined. (' + str(
            (curr_block_id - last_block_id)*txn_per_block) + ' vote transactions have been sealed.)'
...
def dummy_data_input(to_do):
    ...
    if to_do['createPoliticianParties']:
        ...
        for party in parties.values():
            PoliticalParty(
                party_id=party['party_id'], party_name=party['party_name'], party_logo=party['party_logo']).save()
    ...
    if to_do['createRandomVoters']:
        ...
        for i in range(1, no_of_voters+1):
            ...
            dob = datetime.date(random.randint(1980, 2002),
                                random.randint(1, 12), random.randint(1, 28))
            pincode = ''.join(random.choice(string.digits) for _ in range(6))
            region = ''.join(random.choice(
                string.ascii_lowercase + string.ascii_uppercase) for _ in range(20))
            voter = Voters(uuid=uuid, name=name, dob=dob,
                           pincode=pincode, region=region).save()
    ...
    if to_do['castRandomVote'] and to_do['createRandomVoters'] and to_do['createPoliticianParties']:
        ...
        for i in range(1, no_of_voters+1):
            ...
            VoteBackup(uuid=i, vote_party_id=party_id,
                       timestamp=curr_time).save()
...
def block_info(request):
    try:
        ...
        vote_hashes = [SHA3_256.new((f'{vote.uuid}|{vote.vote_party_id}|{vote.timestamp}').encode(
            'utf-8')).hexdigest() for vote in votes]

        root = MerkleTools()
        root.add_leaf(
            [f'{vote.uuid}|{vote.vote_party_id}|{vote.timestamp}' for vote in votes], True)
        root.make_tree()
...
def sync_block(request):
    try:
        ...
        backup_votes = VoteBackup.objects.filter(
            block_id=block_id).order_by('timestamp')
        print(backup_votes)
