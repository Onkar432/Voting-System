...
def send_email_otp(email_to):
    otp = ''.join(random.choice(string.ascii_lowercase +
                  string.ascii_uppercase + string.digits) for _ in range(8))
    msg = EmailMessage()
    msg['From'] = EMAIL_ADDRESS
    msg['To'] = email_to
    msg['Subject'] = 'Don\'t reply, OTP for email verfication'
    content = 'Verify your email id to get the private key to cast your priceless vote. ' + \
        otp + ' is your OTP for email verfication.\nThank you.'
...
def vote_count():
    ...
    vote_result = {party: votes.filter(
        vote_party_id=party).count() for party in parties_id}
    # print(vote_result)
    return vote_result
